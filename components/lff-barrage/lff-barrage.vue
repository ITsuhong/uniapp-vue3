<template>
  <view style="overflow: hidden;position: relative;width: 100%;height: 100%;pointer-events: none; top: 0;">
    <view class="danmu-li" v-for="(item, index) in listData" :class="[item.type,{'danmu-li-flex': type == 'leftBottom'}]" :style="[item.style]" :key="index">
      <view class="danmu-inner">
        <view class="user-box">
          ...
          <view v-if="type == 'leftBottom'" class="user-status cl1" style="max-width: 600rpx">{{ item.item }}</view>
          <view v-else class="user-status cl1" >{{ item.item }}</view>
        </view>
      </view>
    </view>
  </view>
    ...
    horStacked(data) {
      return {
        ...
        style: {
          ...
          top: `${Math.ceil(Math.random() * (this.maxTop - this.minTop - this.hrackH + 1) + this.minTop)}px`
        },
